...
export function WhatsAppLeadModal({ isOpen, onClose, broker, property, source, origin }: WhatsAppLeadModalProps) {
  ...
  const onSubmit = async (data: LeadFormData) => {
    ...
    if (result.success) {
      ...
      // Use window.location.href as requested to avoid popup blockers
      ...
      >
        ...
        <div className="p-6">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <label htmlFor="name-modal" className="sr-only">Nome</label>
              <input
                {...form.register('name')}
                id="name-modal"
            ...
            <div>
              <label htmlFor="phone-modal" className="sr-only">Telefone</label>
              <input
                {...form.register('phone')}
                id="phone-modal"
                className="w-full h-11 px-3 rounded-lg border-gray-200 bg-gray-50 focus:border-primary focus:ring-primary text-sm"
