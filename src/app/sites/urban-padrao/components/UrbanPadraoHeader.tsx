...
export function UrbanPadraoHeader({ broker }: { broker: Broker }) {
  ...
  const servicosUrl = isPortalAccess ? `/sites/${broker.slug}/servicos` : '/servicos';
  const sobreUrl = isPortalAccess ? `/sites/${broker.slug}/sobre` : '/sobre';
  const faleConoscoUrl = isPortalAccess ? `/sites/${broker.slug}/fale-conosco` : '/fale-conosco';
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white/90 backdrop-blur-md border-b border-[#f0f2f4]">
      <div className="layout-container flex h-full flex-col mx-auto max-w-[1280px]">
        <div className="flex items-center justify-between px-6 py-2 lg:px-10 h-20 md:h-28">
          <div className="flex items-center gap-3 text-text-main hover:opacity-80 transition-opacity cursor-pointer h-full">
            <Link href={homeUrl} className="flex items-center h-full">
              {broker.logoUrl ? (
                <Image 
                  src={broker.logoUrl} 
                  ...
                  width={800} 
                  height={200} 
                  className="h-auto max-h-[60px] md:max-h-[90px] w-auto object-contain object-left" 
                ...
                <Image 
                  ...
                  width={400} 
                  height={150} 
                  className="h-auto max-h-[40px] md:max-h-[60px] w-auto object-contain object-left" 
                  style={{ width: 'auto', height: 'auto' }} 
                  priority 
                />
              )}
          ...
          <nav className="hidden lg:flex items-center gap-8">
            <Link className={navLinkClasses(homeUrl)} href={homeUrl}>Início</Link>
            <Link className={navLinkClasses(searchUrl)} href={searchUrl}>Imóveis</Link>
            <Link className={navLinkClasses(mapUrl)} href={mapUrl}>Explorar no Mapa</Link>
            <Link className={navLinkClasses(servicosUrl)} href={servicosUrl}>Serviços</Link>
                  ...
                  <SheetContent style={dynamicSheetStyles} side="right" className="p-0 flex flex-col bg-white">
                      <SheetHeader className="p-6 border-b">
                        ...
                        <Link href={homeUrl} onClick={() => setIsMobileMenuOpen(false)}>
                            <Image src={broker.logoUrl || defaultLogo || ""} alt="Logo" width={160} height={40} className="h-[40px] w-auto object-contain" style={{ width: 'auto' }} />
                        </Link>
                      </SheetHeader>
                      <nav className="flex flex-col gap-2 p-4 text-lg font-semibold">
                          <Link href={homeUrl} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 rounded-lg py-3 px-4 hover:bg-gray-100 transition-colors">
                              <span className="material-symbols-outlined">home</span>Início
                          </Link>
                           <Link href={searchUrl} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 rounded-lg py-3 px-4 hover:bg-gray-100 transition-colors">
                              <span className="material-symbols-outlined">real_estate_agent</span>Imóveis
                          </Link>
                          <Link href={mapUrl} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 rounded-lg py-3 px-4 hover:bg-gray-100 transition-colors">
                              <span className="material-symbols-outlined">map</span>Mapa
                          </Link>
                          <Link href={servicosUrl} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 rounded-lg py-3 px-4 hover:bg-gray-100 transition-colors">
                              <span className="material-symbols-outlined">concierge</span>Serviços
                          </Link>
                          <Link href={sobreUrl} onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 rounded-lg py-3 px-4 hover:bg-gray-100 transition-colors">
                      ...
                      <div className="mt-auto p-6 space-y-4 border-t">
                          ...
                          >
                            <Link href={faleConoscoUrl} onClick={() => setIsMobileMenuOpen(false)}>
                              {ctaText}
                            </Link>
                          </Button>
                      </div>
