 ...
 * Versão Definitiva: Sincronização completa com a arquitetura Oraora 1.0 e paridade com Urban Padrão.
 */
    ...
    <div style={dynamicStyles} className="domus-theme font-display bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 min-h-screen transition-colors duration-300 text-left">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-8 flex items-center justify-between text-left">
          <button onClick={() => router.back()} className="flex items-center gap-2 text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors border-none bg-transparent cursor-pointer outline-none">
        ...
        <section className="grid grid-cols-12 gap-4 mb-12 md:h-[550px]">
          <div onClick={() => openGallery(0)} className="col-span-12 md:col-span-8 relative overflow-hidden rounded-[2.5rem] group cursor-pointer shadow-2xl">
            <Image alt="Main" src={property.midia?.[0] || 'https://picsum.photos/seed/main/800/600'} fill className="object-cover transition-transform duration-1000 group-hover:scale-110" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 text-left">
          <div className="lg:col-span-8 space-y-12">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-10 py-10 border-y border-slate-100 dark:border-slate-800">
                    <div className="space-y-1">
                        ...
                        <p className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                            <span className="material-symbols-outlined text-primary text-2xl">square_foot</span> {caracteristicasimovel.tamanho}
                        </p>
                    </div>
                    <div className="space-y-1">
            ...
            <div className="prose prose-slate dark:prose-invert max-w-none text-left">
              ...
              <div className="text-slate-600 dark:text-slate-400 text-xl leading-relaxed font-light" dangerouslySetInnerHTML={{ __html: informacoesbasicas.descricao || '' }} />
            </div>
      ...
      {isGalleryOpen && property.midia && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-black/95 backdrop-blur-md animate-in fade-in duration-300">
          <header className="p-6 flex items-center justify-between text-white border-b border-white/10">
            ...
            <button onClick={closeGallery} className="size-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors border-none bg-transparent cursor-pointer">
              <span className="material-symbols-outlined">close</span>
            </button>
          </header>
          <div className="flex-1 relative flex items-center justify-center p-4">
            <button onClick={() => setSelectedImageIndex(prev => (prev - 1 + property.midia.length) % property.midia.length)} className="absolute left-6 p-4 rounded-full bg-white/5 text-white hover:bg-primary hover:text-black transition-all border-none bg-transparent cursor-pointer"><span className="material-symbols-outlined text-4xl">chevron_left</span></button>
            <div className="relative h-full w-full flex items-center justify-center">
               <Image alt="Full" src={property.midia[selectedImageIndex]} fill className="object-contain" />
            </div>
            <button onClick={() => setSelectedImageIndex(prev => (prev + 1) % property.midia.length)} className="absolute right-6 p-4 rounded-full bg-white/5 text-white hover:bg-primary hover:text-black transition-all border-none bg-transparent cursor-pointer"><span className="material-symbols-outlined text-4xl">chevron_right</span></button>
          </div>
        </div>
      )}
    </div>
