 ...
 * Garante que o cabeçalho e estilos dinâmicos sigam o padrão Domus.
 */
...
export default function DomusMapClientPage({ broker, properties }: { broker: Broker; properties: Property[] }) {
  ...
  return (
    <div style={dynamicStyles} className="domus-theme relative flex min-h-screen w-full flex-col group/design-root bg-background-light dark:bg-background-dark text-text-main font-display antialiased overflow-x-hidden selection:bg-primary selection:text-black">
      ...
      <main className="flex-1 w-full flex relative h-[calc(100vh-64px)] overflow-hidden">
                  ...
                  <div key={property.id} className="flex flex-col bg-white rounded-xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition-all group cursor-pointer" onClick={() => handleMarkerClick(property)}>
                    ...
                    <div className="p-3">
                        <div className="flex justify-between items-start gap-2">
                            ...
                            {property.informacoesbasicas.valor && <span className="text-primary font-black text-sm whitespace-nowrap">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', minimumFractionDigits: 0, maximumFractionDigits:0 }).format(property.informacoesbasicas.valor)}</span>}
                        </div>
        ...
        {selectedProperty && (() => {
            const propertySlugOrId = selectedProperty.informacoesbasicas.slug || selectedProperty.id;
            const propertyUrl = isPortalAccess 
                ? `/sites/${broker.slug}/imovel/${propertySlugOrId}` 
                : `/imovel/${propertySlugOrId}`;
                ...
                <div className="absolute inset-0 z-40 flex items-center justify-center p-4 bg-black/20" onClick={handleCloseModal}>
                    <div className="bg-white w-full max-w-[400px] rounded-2xl shadow-float overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200 relative" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="p-5 flex flex-col gap-3">
                            ...
                            <Link 
                                href={propertyUrl} 
                                className="flex-1 bg-primary hover:bg-primary-hover text-black font-bold h-11 rounded-xl shadow-lg shadow-primary/20 flex items-center justify-center gap-2 transition-all hover:scale-[1.02]"
                            >
                                Ver Detalhes
